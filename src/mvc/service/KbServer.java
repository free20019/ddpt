...
//		}
	...
	public String findfd(String keyword) {
		String sql = "select t.COMP_ID,t.UNIT_NAME from TB_COMPANY t where t.UNIT_NAME is not null and t.UNIT_NAME like '%"+keyword+"%'";
		List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
		return jacksonUtil.toJson(list);
	}
...
//            System.out.println(sql);
	...
	public String queryaxyw(String postData) {
//		System.out.println(postData);
		Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
		String ywbh = String.valueOf(paramMap.get("ywbh"));
		String pcch = String.valueOf(paramMap.get("pcch"));
		...
		String etime = String.valueOf(paramMap.get("etime"));
		String szqy = String.valueOf(paramMap.get("szqy"));
		String ycms = String.valueOf(paramMap.get("ycms"));
		String scstime = String.valueOf(paramMap.get("scstime"));
		String scetime = String.valueOf(paramMap.get("scetime"));
		String ddlx = String.valueOf(paramMap.get("ddlx"));
		...
		if(!pcch.equals("")){
			cxtj +=" and (t.vehi_no1 like '%"+pcch+"%' or t.vehi_no2 like '%"+pcch+"%' or t.vehi_no3 like '%"+pcch+"%')";
		}
		if(!khdh.equals("")){
			cxtj +=" and t.cust_tel like '%"+khdh+"%'";
		}
		...
		if(!cxgjz.equals("")){
			cxtj +=" and (t.disp_id like '%"+cxgjz+"%' or t.vehi_no1 like '%"+cxgjz+"%' or t.vehi_no2 like '%"+cxgjz+"%' or t.vehi_no3 like '%"+cxgjz+"%' or t.cust_tel like '%"+cxgjz+"%' or t.cust_name like '%"+cxgjz+"%' or t.dest_address like '%"+cxgjz+"%' or t.disp_state like '%"+cxgjz+"%' or t.note like '%"+cxgjz+"%' or t.address like '%"+cxgjz+"%' or t.szqy like '%"+cxgjz+"%') ";
		}
		if(!szqy.equals("")){
			cxtj +=" and t.SZQY='"+szqy+"'";
		}
		...
		if(!stime.equals("")){
			cxtj +=" and t.disp_time>=to_date('"+stime+"','yyyy-mm-dd hh24:mi:ss')";
		}
		if(!etime.equals("")){
			cxtj +=" and t.disp_time<=to_date('"+etime+"','yyyy-mm-dd hh24:mi:ss')";
		}
		if(!ycms.equals("")){
			cxtj +=" and t.YCMS='"+ycms+"'";
		}
		if(!scstime.equals("")){
			cxtj +=" and t.db_time>=to_date('"+scstime+"','yyyy-mm-dd hh24:mi:ss')";
		}
		if(!scetime.equals("")){
			cxtj +=" and t.db_time<=to_date('"+scetime+"','yyyy-mm-dd hh24:mi:ss')";
		}
		if(!"".equals(ddlx)){
			cxtj +=" and t.add_ways='"+ddlx+"'";
		}

		String sql = "select t.*,case when add_ways='1' then '分中心订单' else '调度平台订单' end ddlx from tb_dispatch_love t where t.del_flag='0' "+cxtj+" order by t.disp_time desc";
//		if(!zdlx.equals("")||!ssgs.equals("")){
//			sql = "select t.*,t1.comp_name gsmc,t1.mt_name zdlx from tb_dispatch_love t,vw_vehicle t1 where t.vehi_no=t1.vehi_no and t1.comp_name like '%"+ssgs+"%' and t1.mt_name like '%"+zdlx+"%' and t.disp_time>=to_date('"+stime+"','yyyy-mm-dd hh24:mi:ss') and t.disp_time<=to_date('"+etime+"','yyyy-mm-dd hh24:mi:ss') "+cxtj+" order by t.disp_time desc";
//		}else{
//			sql = "select tt.*,t1.comp_name gsmc,t1.mt_name zdlx from ("+sql+") tt left join vw_vehicle t1 on tt.vehi_no=t1.vehi_no";
//		}
//		System.out.println(sql);
	...
	public List<Map<String, Object>> queryaxyw_daochu(String postData) {
//		System.out.println(postData);
	    Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
        String ywbh = String.valueOf(paramMap.get("ywbh"));
        String pcch = String.valueOf(paramMap.get("pcch"));
...
//        }
		...
		for (int i = 0; i < result.size(); i++) {
			...
			m.put("CALL_STATE", formatTZZT(callstate));
			m.put("AUTOOUTPHONE", formatSFWB(aotuoutphone));
			m.put("DISP_TIME",m.get("DISP_TIME")==null?"":m.get("DISP_TIME").toString().substring(0, 19));
			m.put("DB_TIME",m.get("DB_TIME")==null?"":m.get("DB_TIME").toString().substring(0, 19));
		}
		return result;
	}
...
//		System.out.println(sql);
	...
	public String queryzx(String postData) {
		...
		if(!comp.equals("")){
			tj +=" and t.company like '%"+comp+"%'";
		}
		if(!cllx.equals("")){
			tj +=" and t.cs_type = '"+cllx+"'";
		}
		if(!clzt.equals("")){
			tj +=" and t.cs_state = '"+clzt+"'";
		}
...
//		System.out.println(sql);
	...
	public String queryQyjdtj(String postData) {
		Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
		String stime = String.valueOf(paramMap.get("stime"));
		String etime = String.valueOf(paramMap.get("etime"));
		String gs = String.valueOf(paramMap.get("gs"));
		String fd = String.valueOf(paramMap.get("fd"));
		List<Map<String,Object>> result = new ArrayList<Map<String, Object>>();
		List<Map<String,String>> sjd = getSJD(stime,etime);
		String sql="select ? rq,t3.comp_name gsmc,t3.unit_name fdmc,count(1) num from tb_dispatch_love t1,vw_vehicle t2,tb_company t3 "+
					"where t1.vehi_no1=t2.vehi_no "+
					"and t2.comp_id=t3.comp_id "+
					"and t1.ycms='结对用车' "+
					"and t1.AUDIT_STATUS='0' "+
					"and t1.disp_time>=to_date(?,'yyyy-mm-dd hh24:mi:ss') "+
					"and t1.disp_time<=to_date(?,'yyyy-mm-dd hh24:mi:ss') "+
					"and t3.comp_name like ? "+
					"and t3.unit_name like ? "+
					"group by t3.comp_name,t3.unit_name";
		for (int i = 0; i < sjd.size(); i++) {
			String rq = sjd.get(i).get("rq");
			String sd = sjd.get(i).get("stime");
			String ed = sjd.get(i).get("etime");
			List<Map<String, Object>> list = jdbcTemplate.queryForList(sql,rq,sd,ed,"%"+gs+"%","%"+fd+"%");
			result.addAll(list);
		}
		return jacksonUtil.toJson(result);
	}

	public String queryCljdtj(String postData) {
		Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
		String stime = String.valueOf(paramMap.get("stime"));
		String etime = String.valueOf(paramMap.get("etime"));
		String gs = String.valueOf(paramMap.get("gs"));
		...
		List<Map<String,Object>> result = new ArrayList<Map<String, Object>>();
		List<Map<String,String>> sjd = getSJD(stime,etime);
		String sql="select tt.*,t4.driver_name jsy from (select ? rq,t3.comp_name gsmc,t1.vehi_no1 cphm,count(1) num " +
				"from tb_dispatch_love t1,vw_vehicle t2,tb_company t3 "+
					"where t1.vehi_no1=t2.vehi_no "+
					"and t2.comp_id=t3.comp_id "+
					"and t1.ycms='结对用车' "+
					"and t1.AUDIT_STATUS='0' "+
					...
					"and t1.disp_time<=to_date(?,'yyyy-mm-dd hh24:mi:ss') "+
					"and t3.comp_name like ? "+
					"and t1.vehi_no1 like ? "+
					"group by t1.vehi_no1,t3.comp_name) tt left join tb_unit_driver t4 on tt.cphm=vehicle_no";
		for (int i = 0; i < sjd.size(); i++) {
			String rq = sjd.get(i).get("rq");
			String sd = sjd.get(i).get("stime");
			String ed = sjd.get(i).get("etime");
			List<Map<String, Object>> list = jdbcTemplate.queryForList(sql,rq,sd,ed,"%"+gs+"%","%"+cp+"%");
			result.addAll(list);
		}
		return jacksonUtil.toJson(result);
	}
	...
	public static List<Map<String,String>> getSJD(String stime,String etime){
		...
		try {
			...
			if (sd.getTime() == ed.getTime()) {
				...
				m.put("rq",sdf.format(sd));
				m.put("stime",stime);
				m.put("etime",etime);
				result.add(m);
			}else if(ed.getTime()>sd.getTime()){
				Map<String,String> m1 = new HashMap<String, String>();
				m1.put("rq",sdf.format(sd));
				m1.put("stime",stime);
				m1.put("etime",sdf.format(sd)+" 23:59:59");
				result.add(m1);
...
//						m2.put(sdf.format(cal.getTime()),sdf.format(cal.getTime())+" 00:00:00,"+sdf.format(cal.getTime())+" 23:59:59");
						m2.put("rq",sdf.format(cal.getTime()));
						m2.put("stime",sdf.format(cal.getTime())+" 00:00:00");
						m2.put("etime",sdf.format(cal.getTime())+" 23:59:59");
						result.add(m2);
					} else {
						Map<String,String> m3 = new HashMap<String, String>();
//						m3.put(sdf.format(cal.getTime()),sdf.format(cal.getTime())+" 00:00:00,"+etime);
						m3.put("rq",sdf.format(cal.getTime()));
						m3.put("stime",sdf.format(cal.getTime())+" 00:00:00");
						m3.put("etime",etime);
						result.add(m3);
						break;
					}
	...
	public List<Map<String, Object>> queryQyjdtj_daochu(String postData) {
		Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
		String stime = String.valueOf(paramMap.get("stime"));
		String etime = String.valueOf(paramMap.get("etime"));
		String gs = String.valueOf(paramMap.get("gs"));
	...
	public List<Map<String, Object>> queryCljdtj_daochu(String postData) {
		Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
		String stime = String.valueOf(paramMap.get("stime"));
		String etime = String.valueOf(paramMap.get("etime"));
		String gs = String.valueOf(paramMap.get("gs"));
