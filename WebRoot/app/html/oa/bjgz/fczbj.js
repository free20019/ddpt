define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		return declare( null,{
			...
			get: function () {
				this.getCzrzTable({"time":$("#fczbj-date").val(),"gjz":$("#fczbj-gjz").val(),"table":"TB_OA_NMALVEHI","type":$("#fczbj-type").val()});
			},
			getCzrzTable: function(json) {
				var _self = this;
				xhrArgsTabel.load = function(data) {
					$("#fczbj-infoPanel dd").remove();
					for(var i=0; i<data.length; i++){
						user = data[0].USER;
						var tab = '<dd class="iFItem iFContent">'+
									    '<div class="iFTabularPanel">'+
								        '<div class="iFTitle">'+
								            '<input type="checkbox" class="iFCheckBox iFCheckboxItem" name="fczbj-Cbdelete" value = "'+data[i].OMV_ID+'">'+
								            '<label>记录编号</label>'+
								    ...
								    '</div>'+
								'</dd>';
								$(tab).data('base', data[i]).appendTo('#fczbj-infoPanel');
					}
					$(".iFTabularPanel .iFBtnEditor").on('click', function () {
						fczbjDialogPanel.set('href', 'app/html/oa/editor/bjgzEditor.html');
						var base = $(this).parents('dd').data('base');
						fczbjDialog.show().then(function () {
							fczbjDialog.set('title', '修改');
							...
							$("#fczbjDialogPanel #bjgz-jlsj").val(new Date(base.DATETIME).format('yyyy-MM-dd hh:mm:ss'));
			...
			initToolBar:function(){
				// query('#fczbjPanel .iFComboBox').on('click', function () {
				// 	var thisOne = this;
				// 	if ($(this).hasClass('selected')) {
				// 		$(this).removeClass('selected');
				// 	} else {
				...
				// 	}
				// });
				addEventComboBox('#fczbjPanel');
				var _self = this;
				query('#fczbjQuery').on('click', function () {
					_self.getCzrzTable({"time":$("#fczbj-date").val(),"gjz":$("#fczbj-gjz").val(),"table":"TB_OA_NMALVEHI","type":$("#fczbj-type").val()});
				});
				query('#fczbj-dcsj').on('click', function () {
					var postData = {"time":$("#fczbj-date").val(),"gjz":$("#fczbj-gjz").val(),"table":"TB_OA_NMALVEHI","type":$("#fczbj-type").val()};
						url = "OA/bjgzexcle?postData="
							+ encodeURI(encodeURI(JSON.stringify(postData))), window.open(url)
				});
				query('#fczbj-AllDelete').on('click', function () {
					var id_array=new Array();  
					$('input[name="fczbj-Cbdelete"]:checked').each(function(){  
					    id_array.push($(this).val());//向数组中添加元素
					});  
					var idstr=id_array.join(',');//将数组元素连接起来以构建一个字符串
					_self.deltable(idstr);
				});
				query('#fczbj-date').on('focus', function () {
					WdatePicker({dateFmt: STATEYEARMONTH});
				});

				query('#fczbj-allSelect').on('change', function () {
				...
				query('#fczbj-infoPanel .iFCheckboxItem').on('change', function () {
					var state = $(this).prop('checked'),
					...
					var allCheckBox = $('#fczbj-allSelect');
            ...
            eventEditor: function (type) {
							var id = '#bjgz', info = oaDialogInfo[type]('#fczbj', '非出租故障');
							query(info.id).on('click', function () {
								fczbjDialogPanel.set('href', 'app/html/oa/editor/bjgzEditor.html');
								fczbjDialog.show().then(function () {
									fczbjDialog.set('title', info.title);
									$("#fczbjDialogPanel #bjgz-jlsj").val(new Date().format('yyyy-MM-dd hh:mm:ss'));
            ...
            deltable : function(id){
            	var xhrArgsdel = {
						...
						postData :{"id":id,"table":"TB_OA_NMALVEHI"},
						handleAs : "json",
						Origin : self.location.origin,
						preventCache : true,
						withCredentials : true,
