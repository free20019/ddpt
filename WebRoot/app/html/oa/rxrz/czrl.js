define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		return declare( null,{
			...
			get: function () {
				this.getCzrzTable({"time":$("#czrl-date").val(),"gjz":$("#czrl-gjz").val(),"table":"TB_OA_LOG"});
			},
			getCzrzTable: function(json) {
				var _self = this;
				xhrArgsTabel.load = function(data) {
					$("#czrl-infoPanel dd").remove();
					for(var i=0; i<data.length; i++){
						user = data[0].USER;
						var tab = '<dd class="iFItem iFContent"><div class="iFTabularPanel">'+
				                    '<div class="iFTitle">'+
					                    '<input type="checkbox" name="Cbdelete" class="iFCheckBox iFCheckboxItem" value = "'+data[i].OLO_ID+'">'+
					                    '<label>记录编号</label>'+
					                ...
					                '</div>'+
				                '</dd>';
								$(tab).data('base', data[i]).appendTo('#czrl-infoPanel');
					}
			...
			initToolBar:function(){
				var _self = this;
				query('#czrlQuery').on('click', function () {
					_self.getCzrzTable({"time":$("#czrl-date").val(),"gjz":$("#czrl-gjz").val(),"table":"TB_OA_LOG"});
				});
				query('#czrl-date').on('focus', function () {
					WdatePicker({dateFmt: STATEYEARMONTH});
				});
				query('#czrl-datetime').on('focus', function () {
					WdatePicker({dateFmt: STATEDATE});
				});
				query('#AllDelete').on('click', function () {
				...
				query('#czrl-excel').on('click', function () {
					var postData = {
							"time":$("#czrl-date").val(),"gjz":$("#czrl-gjz").val(),"table":"TB_OA_LOG"
						};
						url = "OA/czrzexcle?postData="
							+ encodeURI(encodeURI(JSON.stringify(postData))), window.open(url)
				});
				query('#czrl-allSelect').on('change', function () {
				...
				query('#czrl-infoPanel .iFCheckboxItem').on('change', function () {
					var state = $(this).prop('checked'),
            ...
            eventEditor: function (type) {
                var id = '#rxrz', info = oaDialogInfo[type]('#czrl', '出租日志');
                query(info.id).on('click', function () {
                    ...
                    czrlDialog.show().then(function () {
                        ...
                        /*关闭弹框*/
                        query(id + 'EditorPanel .iFBtnClose').on('click', function () {czrlDialog.hide();});
                    });
                });
            },
            deltable : function(id){
            	var xhrArgsdel = {
						...
						postData :{"id":id,"table":"TB_OA_LOG"},
						handleAs : "json",
						Origin : self.location.origin,
						preventCache : true,
						withCredentials : true,
