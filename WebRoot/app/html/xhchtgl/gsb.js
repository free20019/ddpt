define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		var columns = {
			...
			COM_NAME: {label: '公司名称'},
			COUNT1: {label: '违章次数'},
			IC_SCORE: {label: 'IC卡分值'}
		};
		var xhrArgsTabel = {
			url : basePath + "xhchtgl/findgs",
			postData : 'postData={"page":1,"pageSize":30,"COM_NAME":'+$(".gsmc-list").val()+'}',
			handleAs : "json",
		...
		return declare( null,{
			constructor:function(){
				...
				layer.load(1);
				pageii = new Pagination(gsbGrid,xhrArgsTabel,{"COMP_NAME":$(".gsmc-list").val()},30);
...
//				gsbGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: columns}, 'gsbTabel');
				query('#gsbQuery').on('click', function () {
					...
					$(".pagination").remove();
					pageii = new Pagination(gsbGrid,xhrArgsTabel,{"COM_NAME":$(".gsmc-list").val()},30);
					dc.place(pageii.first(),'gsbTabel','after');
//					pageii.first();
				});
				query('#gsbUpd').on('click', function () {
					...
					gsbDialog.show().then(function() {
						...
						query('#gsbEditorPanel .iFBtnCommit').on('click', function () {
							...
							FormJson.FGS_OWNER_ID = fgs_qk;
							FormJson.FGS_RE_ID = fgs_qy;
							var datap = JSON.stringify(FormJson);
							var reg = new RegExp("\\[","g");
							var reg1 = new RegExp("\\]","g");
							datap = datap.replace(reg,"").replace(reg1,"");
							var xhrArgs2 = {
				...
				query('#gsbDc').on('click', function() {
					var postData = {
							"COM_NAME":$(".gsmc-list").val()
						};
						url = "backExcel/findgswzbexcle?postData="
