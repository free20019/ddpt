	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		return declare( null,{
			...
			initToolBar:function(){
				var _self = this;
				if (jetjGrid) { jetjGrid = null; dojo.empty('jetjTabel') }
				jetjGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: columns,allowTextSelection: true}, "jetjTabel");
				for (var i = 0; i < data.data.length; i++) {
					data.data[i] = dojo.mixin({dojoId: i + 1}, data.data[i]);
				}
				store = new Memory({data: {identifier: 'dojoId', label: 'dojoId', items: data.data}});
				jetjGrid.set('collection', store);
				jetjEChart = echarts.init(document.getElementById('jetjEChart'));
				jetjEChart.setOption({
					title: {
						text: '对数轴示例',
						left: 'center'
					},
