define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		return declare( null,{
			...
			get: function () {
				this.getCzrzTable({"time":$("#fczrl-date").val(),"gjz":$("#fczrl-gjz").val(),"table":"TB_OA_NLOG"});
			},
			getCzrzTable: function(json) {
				var _self = this;
				xhrArgsTabel.load = function(data) {
					console.log(data);
					$("#fczrl-infoPanel dd").remove();
					for(var i=0; i<data.length; i++){
						user = data[0].USER;
						var tab = '<dd class="iFItem iFContent"><div class="iFTabularPanel">'+
				                    '<div class="iFTitle">'+
					                    '<input type="checkbox" name="Cbdelete-fczrz" class="iFCheckBox iFCheckboxItem" value = "'+data[i].OLO_ID+'">'+
					                    '<label>记录编号</label>'+
					                ...
					                '</div>'+
				                '</dd>';
								$(tab).data('base', data[i]).appendTo('#fczrl-infoPanel');
					}
			...
			initToolBar:function(){
				var _self = this;
				query('#fczrlQuery').on('click', function () {
					_self.getCzrzTable({"time":$("#fczrl-date").val(),"gjz":$("#fczrl-gjz").val(),"table":"TB_OA_NLOG"});
				});
				query('#fczrl-date').on('focus', function () {
					WdatePicker({dateFmt: STATEYEARMONTH});
				});
				query('#fczrl-datetime').on('focus', function () {
					WdatePicker({dateFmt: STATEDATE});
				});
				query('#fczrl-excel').on('click', function () {
					var postData = {
							"time":$("#fczrl-date").val(),"gjz":$("#fczrl-gjz").val(),"table":"TB_OA_NLOG"
						};
						url = "OA/czrzexcle?postData="
							+ encodeURI(encodeURI(JSON.stringify(postData))), window.open(url)
				});
				query('#fcz-AllDelete').on('click', function () {
					var id_array=new Array();  
					$('input[name="Cbdelete-fczrz"]:checked').each(function(){  
					    id_array.push($(this).val());//向数组中添加元素
					});  
					var idstr=id_array.join(',');//将数组元素连接起来以构建一个字符串
					_self.deltable(idstr);
				});
				query('#fczrl-allSelect').on('change', function () {
				...
				query('#fczrl-infoPanel .iFCheckboxItem').on('change', function () {
					var state = $(this).prop('checked'),
					...
					var allCheckBox = $('#czrl-allSelect');
            ...
            eventEditor: function (type) {
							var id = '#rxrz', info = oaDialogInfo[type]('#fczrl', '出租日志');
            ...
            deltable : function(id){
            	var xhrArgsdel = {
						...
						postData :{"id":id,"table":"TB_OA_NLOG"},
						handleAs : "json",
						Origin : self.location.origin,
						preventCache : true,
						withCredentials : true,
