...
//                    + "and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm "
                    + "and SUBSTR(t2.cphm,-6)=t.cphm_new "
        ...
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
            String sql1 = "select count(*) c from jjq"+db+"_1 t, "
                    ...
                    + "and SUBSTR(t2.cphm,-6)=t.cphm_new "+tj+"";
...
//                + "and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm "+tj+" order by t.shangche desc)tt)ttt where ttt.r<="+er+" and ttt.r>="+sr;
                + "and SUBSTR(t2.cphm,-6)=t.cphm_new "+tj+" order by t.shangche desc)tt)ttt where ttt.r<="+er+" and ttt.r>="+sr;
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
        String sql1 = "select count(*) c from jjq"+db+"_1 t, "
                + "jjq_company t2 where "
                + "JIAOYITYPE='刷卡交易' and t.shangche <=to_date('"+etime+"','yyyy-mm-dd hh24:mi:ss') "
...
//                    + "and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm"+tj+")tt where tt.r<="+er+" and tt.r>="+sr;
                    + "and SUBSTR(t2.cphm,-6)=t.cphm_new"+tj+")tt where tt.r<="+er+" and tt.r>="+sr;
            List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
            String sql1 = "select count(*) c from jjq"+db+"_1 t, "
                            + "jjq_company t2 where "
                            + "t.shangche <=to_date('"+etime+"','yyyy-mm-dd hh24:mi:ss') "
...
//                        "and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm "+tj+"  group by t2.cphm,t2.fgs )tt)ttt where ttt.r<="+er+" and ttt.r>="+sr;
                        "and SUBSTR(t2.cphm,-6)=t.cphm_new "+tj+"  group by t2.cphm,t2.fgs )tt)ttt where ttt.r<="+er+" and ttt.r>="+sr;
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
...
//                        "and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm "+tj+"  group by t2.cphm,t2.fgs";
                        "and SUBSTR(t2.cphm,-6)=t.cphm_new "+tj+"  group by t2.cphm,t2.fgs";
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
        for (int i = 0; i < list.size(); i++) {
...
//        String sql = "select * from (select tt.*,t1.fgs,rownum r from (select CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp,sum(t.tjcs) jycs from jjq_tj_"+db+"_day t where "
        String sql = "select * from (select tt.*,t1.fgs,t1.cphm,rownum r from (select t.cphm_new cp,sum(t.tjcs) jycs from jjq_tj_"+db+"_day t where "
...
//                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where tt.cp=t1.cphm and tt.jycs>="+mincs+" and tt.jycs<="+maxcs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where tt.cp=SUBSTR(t1.cphm,-6) and tt.jycs>="+mincs+" and tt.jycs<="+maxcs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
...
//            tj+=" and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END = '"+cp+"'";
        ...
        String db = stime.substring(0,4)+stime.substring(5, 7);
        Map<String, Object> result =new HashMap<String,Object>();
        String sql = "select tt.*,t1.cphm,t1.fgs,rownum r from (select " +
//                "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp," +
                "t.cphm_new cp," +
                "sum(t.tjcs) jycs from jjq_tj_"+db+"_day t where "
                + "t.type=5 and to_date(t.day,'yyyymmdd')<=to_date('"+etime+"','yyyy-mm-dd') and "
                + " to_date(t.day,'yyyymmdd')>=to_date('"+stime+"','yyyy-mm-dd') "+tj
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where " +
//                "tt.cp=t1.cphm " +
                "SUBSTR(t1.cphm,-6)=tt.cp " +
                "and tt.jycs>="+mincs+" and tt.jycs<="+maxcs;
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
        return list;
    }
...
//            tj+=" and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END = '"+cp+"'";
        ...
        String sql = "select * from (select tt.*,t1.cphm,t1.fgs,rownum r from (select " +
//                "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp," +
                "t.cphm_new cp," +
                "sum(t.jine) jyje from jjq_tj_"+db+"_day t where "
                + "t.type=5 and to_date(t.day,'yyyymmdd')<=to_date('"+etime+"','yyyy-mm-dd') and "
                + " to_date(t.day,'yyyymmdd')>=to_date('"+stime+"','yyyy-mm-dd') "+tj
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where " +
//                "tt.cp=t1.cphm " +
                "SUBSTR(t1.cphm,-6)=tt.cp " +
                "and tt.jyje"+fh+cs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
//      System.out.println(sql);
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
...
//                "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp," +
                "t.cphm_new cp," +
                "sum(t.szlc) jyjc from jjq_tj_"+db+"_day t where "
                + "t.type=5 and to_date(t.day,'yyyymmdd')<=to_date('"+etime+"','yyyy-mm-dd') and "
                + " to_date(t.day,'yyyymmdd')>=to_date('"+stime+"','yyyy-mm-dd') "+tj
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where " +
//                "tt.cp=t1.cphm " +
                "SUBSTR(t1.cphm,-6)=tt.cp " +
                "and tt.jyjc"+fh+cs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
//      System.out.println(sql);
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
...
//                "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp," +
                "t.cphm_new cp," +
                "sum(t.dhsj) jyddsj from jjq_tj_"+db+"_day t where "
                + "t.type=5 and to_date(t.day,'yyyymmdd')<=to_date('"+etime+"','yyyy-mm-dd') and "
                + " to_date(t.day,'yyyymmdd')>=to_date('"+stime+"','yyyy-mm-dd') "+tj
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where " +
//                "tt.cp=t1.cphm " +
                "SUBSTR(t1.cphm,-6)=tt.cp " +
                "and tt.jyddsj"+fh+cs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
//      System.out.println(sql);
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
...
//                "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp," +
                "t.cphm_new cp," +
                "sum(t.kslc) jyks from jjq_tj_"+db+"_day t where "
                + "t.type=5 and to_date(t.day,'yyyymmdd')<=to_date('"+etime+"','yyyy-mm-dd') and "
                + " to_date(t.day,'yyyymmdd')>=to_date('"+stime+"','yyyy-mm-dd') "+tj
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where " +
//                "tt.cp=t1.cphm " +
               "SUBSTR(t1.cphm,-6)=tt.cp " +
                "and tt.jyks"+fh+cs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
//      System.out.println(sql);
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
...
//                "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END cp," +
                "t.cphm_new cp," +
                "sum(t.yssc) jyyysj from jjq_tj_"+db+"_day t where "
                + "t.type=5 and to_date(t.day,'yyyymmdd')<=to_date('"+etime+"','yyyy-mm-dd') and "
                + " to_date(t.day,'yyyymmdd')>=to_date('"+stime+"','yyyy-mm-dd') "+tj
                + " group by t.cphm_new,t.fgs)tt,jjq_company t1 where " +
//                "tt.cp=t1.cphm " +
                "SUBSTR(t1.cphm,-6)=tt.cp " +
                "and tt.jyyysj"+fh+cs+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
//      System.out.println(sql);
        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sql);
...
//                + " t.jsyysj>=to_date('"+stime+" 00:00:00','yyyy-mm-dd hh24:mi:ss') and CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm"+tj
                + " t.jsyysj>=to_date('"+stime+" 00:00:00','yyyy-mm-dd hh24:mi:ss') and SUBSTR(t1.cphm,-6)=t.cphm_new"+tj
...
//                        " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type=5 and t.day='"+stime.replaceAll("-", "")+"'";
                        " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type=5 and t.day='"+stime.replaceAll("-", "")+"'";
        List<Map<String, Object>> list1 = jdbcTemplate2.queryForList(sql1);
...
//                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type=1 and t.day='"+stime.replaceAll("-", "")+"'";
                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type=1 and t.day='"+stime.replaceAll("-", "")+"'";
...
//                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type=2 and t.day='"+stime.replaceAll("-", "")+"'";
                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type=2 and t.day='"+stime.replaceAll("-", "")+"'";
        List<Map<String, Object>> list3 = jdbcTemplate2.queryForList(sql3);

        String sql4 = "select sum(t.dhsj) dhsj, sum(t.tjcs) ryycs,count(distinct(t.cphm_new)) ryycl,sum(t.zlc) rzlc,sum(t.szlc) rzklc,sum(t.kslc) rkslc"+
                ",sum(t.jine) rysje,round(sum(t.yssc)/60,2) ryysc,round(sum(t.tjcs)/count(distinct(t.cphm_new)),0) zzcs,round(sum(t.jine)/count(distinct(t.cphm_new)),2) pjys"+
                ",round(sum(t.szlc)/(sum(t.yssc)/60),2) pjxssd"+
                ",round(sum(t.yssc)/60/count(distinct(t.cphm_new)),2) pjyysc"+
                ",round(sum(t.dhsj)/count(distinct(t.cphm_new)),2) pjdhsj"+
//                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type=6 and t.day='"+stime.replaceAll("-", "")+"'";
                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type=6 and t.day='"+stime.replaceAll("-", "")+"'";
        List<Map<String, Object>> list4 = jdbcTemplate2.queryForList(sql4);
        ...
        double sll4 = Double.parseDouble(list4.get(0).get("RYYCL").toString())/Double.parseDouble(clzslist.get(0).get("C").toString());
        list4.get(0).put("CLZS", clzs);
        list4.get(0).put("SLL", new DecimalFormat("##.0").format(sll4*100)+"%");
        list4.get(0).put("QY", qyname);
        result.put("data1", list1);
        result.put("data2", list2);
        result.put("data3", list3);
        result.put("data4", list4);
        return jacksonUtil.toJson(result);
    }
...
//                        " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type="+sjd+" group by t.day order by t.day";
                        " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type="+sjd+" group by t.day order by t.day";
        List<Map<String, Object>> list1 = jdbcTemplate2.queryForList(sql1);
...
//                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type="+sjd;
                " from jjq_tj_"+db+"_day t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type="+sjd;
        List<Map<String, Object>> list2 = jdbcTemplate2.queryForList(sql2);
...
//                        " from yysjtable t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type="+sjd+" " +
                        " from yysjtable t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type="+sjd+" " +
...
//        String zjsql="select t.* from yysjtable t,JJQ_COMPANY t1 where "+qutj+" CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type="+sjd+
        String zjsql="select t.* from yysjtable t,JJQ_COMPANY t1 where "+qutj+" SUBSTR(t1.cphm,-6)=t.cphm_new and t.type="+sjd+
				" and t.ksyysj>=to_date('"+stime+" 00:00:00','yyyy-mm-dd hh24:mi:ss') " +
				" and t.ksyysj<=to_date('"+etime+" 23:59:59','yyyy-mm-dd hh24:mi:ss')";
        String zjtable="";
 		if(db.equals(db2)){
...
//					" from " + db + " t,JJQ_COMPANY t1 where " + qutj + " CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.type=" + sjd);
					" from " + db + " t,JJQ_COMPANY t1 where " + qutj + " SUBSTR(t1.cphm,-6)=t.cphm_new and t.type=" + sjd);
			if (i < yfzj - 1) {
...
//                        " from jjq_tj_"+db+"_day t,jjq_company t2 where CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm and"+tj+gstj+" t.type=5 and  t.day <='"+etime.replaceAll("-", "")+"' and t.day >='"+stime.replaceAll("-", "")+"' group by t2.fgs";
                        " from jjq_tj_"+db+"_day t,jjq_company t2 where SUBSTR(t2.cphm,-6)=t.cphm_new and"+tj+gstj+" t.type=5 and  t.day <='"+etime.replaceAll("-", "")+"' and t.day >='"+stime.replaceAll("-", "")+"' group by t2.fgs";
...
//                " from jjq_tj_"+db+"_day t,jjq_company t2 where CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t2.cphm and"+tj+gstj+" t.type=5 and  t.day <='"+etime.replaceAll("-", "")+"' and t.day >='"+stime.replaceAll("-", "")+"' group by t2.fgs";
    ...
    public String cltj(String postData) {
        ...
        String px = String.valueOf(paramMap.get("px"));
        if(ds.equals("∞")){
            ds="9999999";
        }
        int page = Integer.valueOf(String.valueOf(paramMap.get("page")==null?"1":paramMap.get("page")));
...
//        String cpddtj="CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm";
        String cpddtj="SUBSTR(t1.cphm,-6)=t.cphm_new";

        String db = stime.substring(0,4)+stime.substring(5, 7);
        Map<String, Object> result =new HashMap<String,Object>();
        
        if(!"0".equals(px)){
        	er = Integer.parseInt(px);
        	sr = 1;
		}
        ...
        String sql = "select * from (select tt.*,rownum r from (select t1.fgs fgs,t1.cphm cphm,t1.zdxh zdlx,"
                ...
                + "and t.shangche >=to_date('"+stime+"','yyyy-mm-dd hh24:mi:ss') "
                + "and "+cpddtj+" "
                +tj+" group by t1.cphm,t1.fgs,t1.zdxh order by count(t.cphm_new) desc)tt where tt.ryycs>="+xs+" and tt.ryycs<="+ds+")ttt where ttt.r<="+er+" and ttt.r>="+sr;
...
//        String cpddtj="CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm";
        ...
        String sqlend="";
        String sql = "select tt.*,rownum r  from (select t1.fgs fgs,t1.cphm cphm,t1.zdxh zdlx,"
                + "sum(to_number(substr(t.denghou,1,2))*60+to_number(substr(t.denghou,3,2))+round(to_number(substr(t.denghou,5,2))/60,2)) dhsj, "
                + "count(t.cphm_new) ryycs,"
                + "to_number(1) ryycl,"
                + "sum(to_number(t.jicheng)/10+to_number(t.kongshi)/10) rzlc, "
                ...
                + "and t.shangche >=to_date('"+stime+"','yyyy-mm-dd hh24:mi:ss') "
                + "and "+cpddtj+" "
                +tj+" group by t1.cphm,t1.fgs,t1.zdxh order by count(t.cphm_new) desc)tt where tt.ryycs>="+xs+" and tt.ryycs<="+ds;

        if(!"0".equals(px)){
        	int er = Integer.parseInt(px);
        	sqlend="select * from ("+sql+")ttt where ttt.r<="+er+" and ttt.r>=0";
		}else{
        	sqlend=sql;
		}

        List<Map<String, Object>> list = jdbcTemplate2.queryForList(sqlend);
        for (int i = 0; i < list.size(); i++) {
            Map<String, Object> m = list.get(i);
            m.put("QY", setqy(m.get("CPHM").toString()));
        }
...
//                    + "CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END ch,"
                    + "t.cphm_new ch,"
                    + "NVL(count(*),0) yyts "+
                      "from JJQ_TJ_"+db+"_DAY t where t.type = '5' group by t.cphm_new) t on t.ch = SUBSTR(t1.cphm,-6) "
...
//			System.out.println(gss);
	...
	public String queryjjr(String postData) {
        Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
        String jjr = String.valueOf(paramMap.get("jjr"));
        String sjd = String.valueOf(paramMap.get("sjd"));
        String qy = String.valueOf(paramMap.get("qy"));
        String snf = String.valueOf(paramMap.get("snf"));
        String enf = String.valueOf(paramMap.get("enf"));
        String qutj="";
        String qyname=qy;
        String cltj="";
        String qyid="";
...
//		String jn = sdf.format(new Date());
//		String jn = nf;
        ////		String bn = Integer.parseInt(jn)-1+"";
        ////		String qn = Integer.parseInt(jn)-2+"";

		//查询该节假日所有日期
		String jjrsql = "select * from tb_holiday where (h_year>=? and h_year<=?) and h_name=? order by h_date ";
//		String jjrsql = "select * from tb_holiday where (h_year=? or h_year=?) and h_name=? order by h_date ";
//		List<Map<String,Object>> jjrlist = jdbcTemplate.queryForList(jjrsql,jn,bn,qn,jjr);
		List<Map<String,Object>> jjrlist = jdbcTemplate.queryForList(jjrsql,snf,enf,jjr);

		StringBuffer sql1 = new StringBuffer();
		for (int i = 0; i < jjrlist.size(); i++) {
			String jjrname = jjrlist.get(i).get("H_NAME").toString();
			String jjrdate = jjrlist.get(i).get("H_DATE").toString().substring(0,10);
			String jjrmark = jjrlist.get(i).get("MARK").toString();
			if(jjrmark.equals("1")){
				jjrname+="(正)";
			}
			String yf = jjrdate.substring(0,4)+jjrdate.substring(5,7);
			String db = "jjq_tj_" + yf + "_day";
			String tableSql="select count(*) c from user_tables where table_name =upper(?)";
			List<Map<String, Object>> tableList = jdbcTemplate2.queryForList(tableSql,db);
			if("1".equals(tableList.get(0).get("C").toString())){
                sql1.append("select '" + jjrname + "' jjr,'" + jjrdate + "' yf,sum(t.dhsj) dhsj, sum(t.tjcs) ryycs,count(distinct(t.cphm_new)) ryycl,sum(t.zlc) rzlc,sum(t.szlc) rzklc,sum(t.kslc) rkslc" +
					",sum(t.jine) rysje,round(sum(t.yssc)/60,2) ryysc,round(sum(t.tjcs)/count(distinct(t.cphm_new)),0) zzcs,round(sum(t.jine)/count(distinct(t.cphm_new)),2) pjys" +
					",round(sum(t.szlc)/(sum(t.yssc)/60),2) pjxssd" +
					",round(sum(t.yssc)/60/count(distinct(t.cphm_new)),2) pjyysc" +
					",round(sum(t.dhsj)/count(distinct(t.cphm_new)),2) pjdhsj" +
//					" from " + db + " t,JJQ_COMPANY t1 where " + qutj + " CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.day='"+jjrdate.replaceAll("-", "")+"' and t.type=" + sjd);
					" from " + db + " t,JJQ_COMPANY t1 where " + qutj + " SUBSTR(t1.cphm,-6)=t.cphm_new and t.day='"+jjrdate.replaceAll("-", "")+"' and t.type=" + sjd);
            }
			if (i < jjrlist.size() - 1) {
				sql1.append(" union all ");
			}
...
//        System.out.println(sql1.toString());
		...
		for (int i = 0; i < list1.size(); i++) {
			String clzssql = "select count(1) c from vw_vehicle t where t.vehi_date<=to_date('"+list1.get(i).get("YF").toString()+"','yyyy-mm-dd') and (t.vehistatus<>'报停' or t.vehistatus is null) and t.vehi_no is not null "+cltj;
			List<Map<String, Object>> clzslist = jdbcTemplate.queryForList(clzssql);
			String clzs = clzslist.get(0).get("C").toString();
			double sll1 = Double.parseDouble(list1.get(i).get("RYYCL").toString())/Double.parseDouble(clzs);
			list1.get(i).put("dojoId", i+1);
    ...
    public List<Map<String, Object>> jjr_daochu(String postData) {
    	Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
        String jjr = String.valueOf(paramMap.get("jjr"));
        String sjd = String.valueOf(paramMap.get("sjd"));
        String qy = String.valueOf(paramMap.get("qy"));
		...
		//查询该节假日所有日期
//		String jjrsql = "select * from tb_holiday where (h_year=? or h_year=? or h_year=?) and h_name=? order by h_date ";
		String jjrsql = "select * from tb_holiday where (h_year=? or h_year=?) and h_name=? order by h_date ";
//		List<Map<String,Object>> jjrlist = jdbcTemplate.queryForList(jjrsql,jn,bn,qn,jjr);
		List<Map<String,Object>> jjrlist = jdbcTemplate.queryForList(jjrsql,jn,bn,jjr);
...
//					" from " + db + " t,JJQ_COMPANY t1 where " + qutj + " CASE WHEN SUBSTR(t.cphm_new,1,1)='A' THEN '浙'||t.cphm_new ELSE '浙A'||t.cphm_new END=t1.cphm and t.day='"+jjrdate.replaceAll("-", "")+"' and t.type=" + sjd);
	...
	public String findKeyArea() {
    	String sql = "select area_id,area_name from TB_KEY_AREA order by area_name";
		List<Map<String, Object>> result = jdbcTemplate.queryForList(sql);
		return jacksonUtil.toJson(result);
	}


	public String queryjjrqy(String postData) {
        Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
        String jjr = String.valueOf(paramMap.get("jjr"));
        String sjd = String.valueOf(paramMap.get("sjd"));
        String qy = String.valueOf(paramMap.get("qy"));
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
        String jn = sdf.format(new Date());
        String qn = (Integer.parseInt(jn)-1)+"";
//		System.out.println(jn+"  "+qn+"  "+qy+"  "+jjr+"  "+sjd);
        String sql = "select * from tb_key_area_yy where (h_year=? or h_year=?) and area_id=? and h_name=? and type=? and owner_id is null order by h_date";
        List<Map<String, Object>> result = jdbcTemplate.queryForList(sql,jn,qn,qy,jjr,sjd);
        return jacksonUtil.toJson(result);
    }

	public String queryclyb(String postData) {
    	Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
        String rq = String.valueOf(paramMap.get("rq"));
        String qy = String.valueOf(paramMap.get("qy"));
//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy");

		String nian = rq.split("-")[0];
		String yue = rq.split("-")[1];

        String jinnian = nian+"-"+yue;
        String qunian = (Integer.parseInt(nian)-1)+"-"+yue;
        String qiannian = (Integer.parseInt(nian)-2)+"-"+yue;
        String sql = "select tt.area_name,tt.yf,LISTAGG(tt.rq||'|'||tt.area_num, ',') WITHIN GROUP(ORDER BY tt.yf) sj from ("+
				"select t.area_name,t.area_num,to_char(db_time,'yyyy') yf, to_char(db_time,'mm-dd') rq from tb_key_area_num t"+
				" where area_id=? " +
				"and owner_id is null " +
				"and (to_char(db_time,'yyyy-mm')=? or to_char(db_time,'yyyy-mm')=? or to_char(db_time,'yyyy-mm')=?) order by db_time" +
				")tt group by tt.yf,tt.area_name";
//		System.out.println(sql);
        List<Map<String, Object>> result = jdbcTemplate.queryForList(sql,qy,jinnian,qunian,qiannian);
        return jacksonUtil.toJson(result);
	}

	public String queryclnb(String postData) {
    	Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
        String rq = String.valueOf(paramMap.get("rq"));
        String qy = String.valueOf(paramMap.get("qy"));

        String jinnian = rq;
        String qunian = (Integer.parseInt(rq)-1)+"";
        String qiannian = (Integer.parseInt(rq)-2)+"";

		String sql = "select tt.area_name,substr(tt.yf, 1, 4) nf," +
				"LISTAGG(substr(tt.yf, 6, 2) || '|' || tt.nums, ',') WITHIN GROUP (ORDER BY tt.yf) sj " +
				"from (select area_name, sum(area_num) nums, to_char(db_time, 'yyyy-mm') yf " +
				"from tb_key_area_num " +
				"where area_id = ? and owner_id is null " +
				"and (to_char(db_time, 'yyyy') = ? or to_char(db_time, 'yyyy') = ? or to_char(db_time, 'yyyy') = ?) " +
				"group by area_name, to_char(db_time, 'yyyy-mm')) tt " +
				"group by tt.area_name, substr(tt.yf, 1, 4)";
//		System.out.println(sql);
        List<Map<String, Object>> result = jdbcTemplate.queryForList(sql,qy,jinnian,qunian,qiannian);
        return jacksonUtil.toJson(result);
