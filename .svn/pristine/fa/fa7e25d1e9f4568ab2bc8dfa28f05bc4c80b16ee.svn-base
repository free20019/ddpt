define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		var zzddGrid = null,  pcqrGrid=null, ddwcGrid=null,ldhmGrid=null,rwcjkGrid=null,qtywGrid=null,fjclGrid=null,store = null;
		var settime=null;
		var yhqSettime=null;
		...
		var ZZDDcolumns = {
            ...
            DISP_ID: {label: '业务编号',renderCell:function(item){
            	...
            	if(item.DDQY=='爱心出租'){
                  type = dc.create("div", {innerHTML: '<i class="icon-love"></i>'+item.DISP_ID, style:{'text-align':'center','color':'red','position':'relative'}});
				}
                return type;
            }},
            YHQ: {label: '优惠券',renderCell:function(item){
				var type = dc.create("div", {innerHTML: item.YHQ, style:{'text-align':'center'}});
				return type;
			}},
            YEWU_TYPE: {label: '业务类型',renderCell:function(item){
            	var ywlxtd="出租调度";
            	if(item.YEWU_TYPE=="1"){
            		ywlxtd="小货调度";
            	}
            ...
            SZQY: {label: '调度区域',renderCell:function(item){
		...
		var QTYWcolumns={
				QTYWXH: {label: '序号',renderCell:function(item){
		...
		var RWCcolumns={
	            ...
	            ANGLE: {label: '方向',renderCell:function(item){
	                var type = dc.create("div", {innerHTML: dlwz(item.ANGLE), style:{'text-align':'center'}});
	                return type;
	            }},
	            STIME: {label: '定位时间',renderCell:function(item){
	                var type = dc.create("div", {innerHTML: util.formatYYYYMMDDHHMISS(item.STIME), style:{'text-align':'center'}});
	                return type;
	            }}
		};
        ...
        var FJCLcolumns={
            ...
            JL: {label: '车辆距离',renderCell:function(item){
...
//            		$('#xdd_zdcheck').attr('checked',false);// 是否自动回拨
                ...
                $("#rgaxddyc1").css('display','block');
                $("#rgaxddyc2").css('display','block');
                map.setZoom(17);
                if(scmarker){
                    scmarker.setMap(null);
                    scmarker=null;
...
//            	console.log(json.VEHI_NO+"  1111111");
			...
			rwcdw: function (json) {
				...
				for (var i = 0; i < json.length; i++) {
		            ...
		            var content = [];
// content.push("经度："+json[i].PX+" 纬度："+json[i].PY);
			        content.push("时间："+util.formatYYYYMMDDHHMISS(json[i].STIME));
// content.push("速度："+json[i].SPEED+" 方向："+dlwz(json[i].ANGLE)+"
// 状态："+kzc(json[i].STATE));
...
// content.push("区块："+json[i].OWNER_NAME);
			        content.push("<hr/>&nbsp;&nbsp;<a href='javascript:void(0)' class='xxfs'>信息发送</a>"+"&nbsp;&nbsp;<a href='javascript:void(0)' class='jfcx'>积分查询</a>"+"&nbsp;&nbsp;<a href='javascript:void(0)' class='bdcz'>拨打车载</a>");
			        var infoWindow = new AMap.InfoWindow({
			            isCustom: true,  // 使用自定义窗体
			            content: createInfoWindow(title, content.join("<br/>")),
			...
			findywd:function(){
			    ...
			    queryOrder(ywdcp,ywdbh).then(function(data){
	                ...
	                qtywGrid.set('collection', new Memory({data: { identifier: 'QTYWXH', label: 'QTYWXH', items: data.qtyw}}));
...
//							$("#xddddqy").val()
				 ...
				 // end
				/* 正在调度表格 */
				if (zzddGrid) { zzddGrid = null; dojo.empty('xddConent-zzddTable');}
				zzddGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: ZZDDcolumns,allowTextSelection: true}, 'xddConent-zzddTable');

                /* 派车确认 */
				if (pcqrGrid) { pcqrGrid = null; dojo.empty('xddConent-pcqrTable');}
                pcqrGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: PCQRcolumns,allowTextSelection: true}, 'xddConent-pcqrTable');
                /* 任务车监控 */
                if (rwcjkGrid) { rwcjkGrid = null; dojo.empty('xddConent-rwcjkTable');}
                rwcjkGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: RWCcolumns,allowTextSelection: true}, 'xddConent-rwcjkTable');
                /* 调度完成 */
                if (ddwcGrid) { ddwcGrid = null; dojo.empty('xddConent-ddwcTable');}
                ddwcGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: DDWCcolumns,allowTextSelection: true}, 'xddConent-ddwcTable');
                /* 其他业务 */
                if (qtywGrid) { qtywGrid = null; dojo.empty('xddConent-qtywTable');}
                qtywGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: QTYWcolumns,allowTextSelection: true}, 'xddConent-qtywTable');
                /* 附近车辆 */
                if (fjclGrid) { fjclGrid = null; dojo.empty('xddConent-fjclTable');}
                fjclGrid = new CustomGrid({ totalCount: 0, pagination: null, columns: FJCLcolumns,allowTextSelection: true}, 'xddConent-fjclTable');
              ...
              // 派车确认右键菜单
                ...
                zzddGrid.on('.dgrid-row:contextmenu', function(event) {
					...
					if(row.data.DDQY=='爱心出租'&&row.data.DISP_STATE=='调度超时'){
                        pMenu.addChild(new MenuItem({
                            label: "爱心调度(人工调度)",
                            onClick: function(){_self.rgdd(row.data);}
                        }));
					}
	                pMenu.addChild(new MenuItem({
	                    label: "拷贝信息(新增调度)",
...
// content.push("区块："+data.OWNER_NAME);
                ...
                on(query('#rgaxdd'), 'click', function () {
                    ...
                    if($("#rgddzpcl").val()==""){layer.msg("请输入指派车辆！");return;}
                ...
                on(query('#xddbtn'), 'click', function () {
                	...
                	if(xddszqy==""){layer.msg("请输入详细地址所在区域！");return;}
