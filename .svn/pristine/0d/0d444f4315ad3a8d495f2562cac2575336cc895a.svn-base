define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
	...
	function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		return declare( null,{
			...
			get: function () {
				this.getCzrzTable({"time":$("#czbj-date").val(),"gjz":$("#czbj-gjz").val(),"table":"TB_OA_MALVEHI","type":$("#czbj-type").val()});
			},
			getCzrzTable: function(json) {
				...
				xhrArgsTabel.load = function(data) {
					$("#czbj-infoPanel dd").remove();
					for(var i=0; i<data.length; i++){
						user = data[0].USER;
						var tab = '<dd class="iFItem iFContent">'+
									    '<div class="iFTabularPanel">'+
								        '<div class="iFTitle">'+
								            '<input type="checkbox" class="iFCheckBox iFCheckboxItem" name="czbj-Cbdelete" value = "'+data[i].OMV_ID+'">'+
								            '<label>记录编号</label>'+
								    ...
								    '</div>'+
								'</dd>';
								$(tab).data('base', data[i]).appendTo('#czbj-infoPanel');
					}
					$(".iFTabularPanel .iFBtnEditor").on('click', function () {
						czbjDialogPanel.set('href', 'app/html/oa/editor/bjgzEditor.html');
						var base = $(this).parents('dd').data('base');
						czbjDialog.show().then(function () {
							czbjDialog.set('title', '修改');
							...
							$("#czbjDialogPanel #bjgz-jlsj").val(new Date(base.DATETIME).format('yyyy-MM-dd hh:mm:ss'));
			...
			initToolBar:function(){
				// query('#czbjPanel .iFComboBox').on('click', function () {
				// 	var thisOne = this;
				// 	if ($(this).hasClass('selected')) {
				// 		$(this).removeClass('selected');
				// 	} else {
				...
				// 	}
				// });
				addEventComboBox('#czbjPanel');
				var _self = this;
				query('#czbjQuery').on('click', function () {
					_self.getCzrzTable({"time":$("#czbj-date").val(),"gjz":$("#czbj-gjz").val(),"table":"TB_OA_MALVEHI","type":$("#czbj-type").val()});
				});
				query('#czbj-dcsj').on('click', function () {
					var postData = {"time":$("#czbj-date").val(),"gjz":$("#czbj-gjz").val(),"table":"TB_OA_MALVEHI","type":$("#czbj-type").val()};
						url = "OA/bjgzexcle?postData="
							+ encodeURI(encodeURI(JSON.stringify(postData))), window.open(url)
				});
				query('#czbj-date').on('focus', function () {
					WdatePicker({dateFmt: STATEYEARMONTH});
				});
				query('#czbj-AllDelete').on('click', function () {
					var id_array=new Array();  
					$('input[name="czbj-Cbdelete"]:checked').each(function(){  
					    id_array.push($(this).val());//向数组中添加元素
					});  
					var idstr=id_array.join(',');//将数组元素连接起来以构建一个字符串
					_self.deltable(idstr);
				});
				query('#czbj-allSelect').on('change', function () {
				...
				query('#czbj-infoPanel .iFCheckboxItem').on('change', function () {
					var state = $(this).prop('checked'),
					...
					var allCheckBox = $('#czbj-allSelect');
            ...
            eventEditor: function (type) {
                var id = '#bjgz', info = oaDialogInfo[type]('#czbj', '出租报警');
                query(info.id).on('click', function () {
                    czbjDialogPanel.set('href', 'app/html/oa/editor/bjgzEditor.html');
                    czbjDialog.show().then(function () {
                        czbjDialog.set('title', info.title);
                        ...
                        // });
											...
											$("#czbjDialogPanel #bjgz-jlsj").val(new Date().format('yyyy-MM-dd hh:mm:ss'));
                        $("#bjgz-jlr").val(USERS);
                        $("#bjgz-jlr").attr("disabled", true);
                        ...
                        /*取消按钮*/
                        query(id + 'EditorPanel .iFBtnClose').on('click', function () {czbjDialog.hide();});
                    });
                });
            },
            deltable : function(id){
            	var xhrArgsdel = {
						...
						postData :{"id":id,"table":"TB_OA_MALVEHI"},
						handleAs : "json",
						Origin : self.location.origin,
						preventCache : true,
						withCredentials : true,
