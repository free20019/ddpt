define([ "dijit/Dialog", "dgrid/Editor", "dijit/form/Button",
    ...
    function(Dialog, Editor, Button, DateTextBox, TimeTextBox,
		...
		var columns = {
            ...
            UP_TIME: {label: '时间',formatter : util.formatYYYYMMDDHHMISS}
        };
		var xhrArgsTabel = {
...
//				}
				$('#findnewold_stime').val(util.formatYYYYMMDDHHMISS(new Date(new Date().getTime() - 1000*60*60*24*30)));
				$('#findnewold_etime').val(util.formatYYYYMMDDHHMISS(new Date()));
...
//					allowSelectAll: true,
				...
				pageii = new Pagination(newoldGrid,xhrArgsTabel,{"gjz":$("#findnewold_dd").val(),"stime":$("#findnewold_stime").val(),"etime":$("#findnewold_etime").val()},30);
...
//					pageii.first();
				...
				query('#newoldAdd').on('click', function() {
					...
					newoldDialog.show().then(function () {
						newoldDialog.set('title', '新增新旧车牌');
							...
							$('#newold-time').val(util.formatYYYYMMDDHHMISS(new Date()))
				...
				query('#newoldUpd').on('click', function() {
					...
					newoldDialog.show().then(function () {
                        ...
                        $("#newold-time").val(util.formatYYYYMMDDHHMISS(newold_data[sz[0]-1].UP_TIME));
                        query('.newold-iFBtnCommit').on('click', function() {
                        	var FormJson = getFormJson("newoldEditorForm");
...
//                        	}
				...
				query('#newoldDc').on('click', function() {
					var postData = {
							...
							"stime":$("#findnewold_stime").val(),
							"etime":$("#findnewold_etime").val()
						};
