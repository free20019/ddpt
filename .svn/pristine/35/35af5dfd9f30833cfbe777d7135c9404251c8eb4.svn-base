...
public class BackExcelServer {
	...
	public String clbdc(String postData){
		...
		String sql = "select a.*,vt.vt_name,vnc_name,vs.vs_name,ct.CT_NAME,t.owner_name vnt_name"
	...
	public String findclxx(String postData){
		...
		String sql = "select b.* from (select * from tb_global_vehicle where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' AND PLATE_NUMBER LIKE '浙A%') b where 1=1 ";
		sql += tj;
...
//				+ " where REPLACE(gpc.plate_number,'.','')=gv.plate_number and gv.industry=090 and gv.business_scope_code = 1400  AND gv.STATUS_NAME='营运' AND gpc.plate_number LIKE '浙A%'"
	...
	public String findtjb(String postData){
		...
		sql += tj;
		sql+=")) as count, tt.* from (select t.*  from (select distinct gv.COMPANY_ID,gv.COMPANY_NAME,count1,count2,count3 from (select * from tb_global_vehicle where industry=090 and business_scope_code = 1400  AND STATUS_NAME='营运' AND PLATE_NUMBER LIKE '浙A%') gv"
	...
	public List<Map<String, Object>> findsjb(String postData){
		Map<String,Object> paramMap = jacksonUtil.toObject(postData,new TypeReference<Map<String,Object>>() {});
		String gjz = String.valueOf(paramMap.get("gjz"));
		String tj = "";
		if(gjz!=null&&!gjz.isEmpty()&&!gjz.equals("null")&&gjz.length()>0){
		...
		String sql = "select s.*,v.COMP_NAME,v.vt_name,v.rllx"
